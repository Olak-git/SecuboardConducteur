...
import LocationBottomButton from '../../../components/LocationBottomButton';
...
const ItineraireView: React.FC<ItineraireViewProps> = ({ navigation, route }) => {
    ...
    const { curLoc, time, isLoading, coordinate:coords, heading } = currentCoords;
    ...
    const {startingCords, destinationCords, coordinate} = state;
    ...
    const onCenter = () => {
        if(currentCoords.isLoading) {
            // @ts-ignore
            mapRef.current?.animateToRegion({
                ...currentCoords.curLoc,
                latitudeDelta: LATITUDE_DELTA,
                longitudeDelta: LONGITUDE_DELTA
            })
        } else {
    ...
    const getCoordonates = async () => {
        ...
        .catch(error => {
            console.warn('ItineraireView Error1: ', error)
        });
        ...
        .catch(error => {
            console.warn('ItineraireView Error2: ', error)
        });
    }
    ...
    const onStateCourse = async (action: string) => {
        ...
        if(locPermissionDenied) {
            ...
            .catch(error => {
                setVisible(state => ({...state, modal: false}));
                console.log('ItineraireView Error3: ', error)
            })
        }
    }
    ...
    const onAcceptCourse = () => {
        ...
        .catch(error => {
            setVisible(state => ({...state, modal: false}));
            console.log('ItineraireView Error4: ', error)
        })
    }

    const updateCurrentState = (data: any) => setCurrentCoords((prevstate:any) => ({...prevstate, ...data}))
    ...
    const currentCoordonates = async () => {
        ...
        if(locPermissionDenied) {
            ...
            if(!currentCoords.isLoading) {
                ...
                // console.log('Yak');
                const newCoordinate = {latitude, longitude};
                updateCurrentState({heading: heading})
                if(Platform.OS == 'android') {
                    // @ts-ignore
                    markerRef.current?.animateMarkerToCoordinate(newCoordinate, 7000)
                } else {
    ...
    useEffect(() => {
        ...
        return () => {
            if(timer.intervalExists('current-coordonnates')) timer.clearInterval('current-coordonnates');
        }
    }, [])
            ...
            ?
                ...
                <View style={{flex: 1}}>
                    ...
                    >
                        {currentCoords.isLoading && (
                            ...
                            >
                                <RNMarker src={require('../../../assets/images/icons8-street-view-100.png')} heading={heading} />
                            </Marker.Animated>
                        )}
                            ...
                            <>
                                ...
                                <MapViewDirections
                                    ...
                                    onError={errorMessage => {
                                        toast('DANGER', errorMessage.message)
                ...
                ?
                    ...
                    ?
                        ...
                        ?
                            <LocationBottomButton buttonTitle='Accepter la course' pressAction={onAcceptCourse} loading={visible.modal} />
                        :
                            <View style={tw`p-3`}>
                    ...
                    : 
                        stateCourse == 1
                        ?
                            <LocationBottomButton buttonTitle='Démarrer la course' pressAction={() => onStateCourse('start')} loading={visible.modal} />
                        : 
                            stateCourse == 10
                            ?
                                <LocationBottomButton buttonTitle='Course Terminée' pressAction={() => onStateCourse('end')} loading={visible.modal} />
                            : null
                :
